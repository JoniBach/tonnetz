<script lang="ts">
	...
	const CONFIG = {
		...
		innerTriangle: { size: 80, strokeWidth: 0.2, strokeColor: '#222', fillColor: '#111', opacity: 0.8 },
		innerCircle: { diameter: 20, strokeWidth: 0.2, strokeColor: '#222', fillColor: '#111', opacity: 0.9 },
		background: '#1a1a1a'
	} as const;

	const triangleHeight = (CONFIG.baseTriangleSize * Math.sqrt(3)) / 2;
	const spacing = { row: triangleHeight, col: CONFIG.baseTriangleSize / 2 };
	const gridBounds = { width: CONFIG.gridExtent * 2 * spacing.col, height: CONFIG.gridExtent * 2 * spacing.row };
	...
	const createZoomBehavior = () => d3.zoom()
		.scaleExtent([1 / CONFIG.zoomRange, CONFIG.zoomRange])
		.translateExtent([[-gridBounds.width / 2, -gridBounds.height / 2], [gridBounds.width / 2, gridBounds.height / 2]])
		.filter((e: any) => e.button === 2 || e.type === 'wheel' || e.type === 'dblclick')
		.on('zoom', (e: d3.D3ZoomEvent<SVGSVGElement, unknown>) => gridGroup.attr('transform', e.transform));

	function createGrid() {
		gridGroup.selectAll('*').remove();
		const [triangles, vertices, innerTriangles, innerCircles] = Array(4).fill(0).map(() => gridGroup.append('g'));
		...
		vertexPositions.forEach((pos) => createInnerCircle(innerCircles, pos));
	}

	function createTriangle(parent: d3.Selection<SVGGElement, unknown, null, undefined>, pos: { x: number; y: number }, up: boolean) {
		const { x, y } = pos;
	...
	function createVertex(parent: d3.Selection<SVGGElement, unknown, null, undefined>, pos: { x: number; y: number }) {
	...
	function createInnerTriangle(parent: d3.Selection<SVGGElement, unknown, null, undefined>, gridPos: { x: number; y: number }, gridUp: boolean) {
		const { x, y } = gridPos;
		const half = CONFIG.baseTriangleSize / 2;
	...
	function createInnerCircle(parent: d3.Selection<SVGGElement, unknown, null, undefined>, pos: { x: number; y: number }) {
		parent.append('circle')
			.attr('cx', pos.x)
			.attr('cy', pos.y)
